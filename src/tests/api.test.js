...
describe("API Testing", () => {
  ...
  it("GET all jobs | Fetch all jobs", async () => {
    ...
    response.data.jobs.forEach((individualJob, index) => {
      expect(individualJob.jobTitle).toEqual(jobs_mock[index].jobTitle);
    });
  });
  ...
  it("GET all application | Fetch all application", async () => {
    ...
    response.data.application.forEach((individualApplication, index) => {
      expect(individualApplication.isVerified).toEqual(application_mock[index].isVerified);
    });
  });
